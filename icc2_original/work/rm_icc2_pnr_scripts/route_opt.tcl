...
if {[file exists [which $TCL_USER_ROUTE_OPT_SCRIPT]]} {
	...
	if {[file exists [which $TCL_USER_ROUTE_OPT_1_POST_SCRIPT]]} {
		puts "RM-info: Sourcing [which $TCL_USER_ROUTE_OPT_1_POST_SCRIPT]"
		source -echo $TCL_USER_ROUTE_OPT_1_POST_SCRIPT
	} elseif {$TCL_USER_ROUTE_OPT_1_POST_SCRIPT != ""} {
		puts "RM-error: TCL_USER_ROUTE_OPT_1_POST_SCRIPT($TCL_USER_ROUTE_OPT_1_POST_SCRIPT) is invalid. Please correct it."
	}	
	...
	if {$ENABLE_IRDCCD} {
		if {[file exists [which $TCL_IRDCCD_CONFIG_FILE]]} {
			puts "RM-info: Running IDR-CCD by sourcing $TCL_IRDCCD_CONFIG_FILE"
                	source $TCL_IRDCCD_CONFIG_FILE
        	} elseif {$TCL_IRDCCD_CONFIG_FILE != ""} {
                	puts "RM-error: IRD-CCD is enabled but TCL_IRDCCD_CONFIG_FILE($TCL_IRDCCD_CONFIG_FILE) is invalid. Please correct it."
		} elseif {$TCL_IRDCCD_CONFIG_FILE == ""} {
                	puts "RM-error: IRD-CCD is enabled but TCL_IRDCCD_CONFIG_FILE is not specified. Please correct it."
