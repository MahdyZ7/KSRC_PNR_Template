...
if {$WRITE_QOR_DATA} {
	...
	## write_qor_data for parallel reporting
	if {$REPORT_PARALLEL_SUBMIT_COMMAND != ""} {

		if {[regexp $REPORT_PREFIX $INIT_DESIGN_BLOCK_NAME]} {
			write_qor_data -report_group mapped -label $REPORT_PREFIX -output $WRITE_QOR_DATA_DIR
		} elseif {[regexp $REPORT_PREFIX $PLACE_OPT_BLOCK_NAME]} {
			write_qor_data -report_group placed -label $REPORT_PREFIX -output $WRITE_QOR_DATA_DIR
		} elseif {[regexp $REPORT_PREFIX "$CLOCK_OPT_CTS_BLOCK_NAME|$CLOCK_OPT_OPTO_BLOCK_NAME"]} {
			write_qor_data -report_group cts -label $REPORT_PREFIX -output $WRITE_QOR_DATA_DIR
		} else {
			write_qor_data -report_group routed -label $REPORT_PREFIX -output $WRITE_QOR_DATA_DIR
		}
	
	} else {
	
		## write_qor_data lite for sequential reporting
		write_qor_data -report_list "host_machine report_app_options report_utilization report_design wirelength" -label $REPORT_PREFIX -output $WRITE_QOR_DATA_DIR
...
if {[regexp $REPORT_PREFIX "$CLOCK_OPT_CTS_BLOCK_NAME|$CLOCK_OPT_OPTO_BLOCK_NAME|$ROUTE_AUTO_BLOCK_NAME|$ROUTE_OPT_BLOCK_NAME|$CHIP_FINISH_BLOCK_NAME|$ICV_IN_DESIGN_BLOCK_NAME|$TIMING_ECO_BLOCK_NAME|$FUNCTIONAL_ECO_BLOCK_NAME"]} {
	if {[get_app_option_value -name time.pocvm_enable_analysis]} {
...
if {$REPORT_QOR_REPORT_POWER} { 
	...
	if {$USE_ABSTRACTS_FOR_BLOCKS != "" && $USE_ABSTRACTS_FOR_POWER_ANALYSIS == "true"} {
		puts "Reading switching activity from saif file ...\n"
#		read_saif ../../../sim_original/testsaif_original_post.saif
...
if {$REPORT_QOR_REPORT_CONGESTION && [regexp $REPORT_PREFIX "$PLACE_OPT_BLOCK_NAME|$CLOCK_OPT_CTS_BLOCK_NAME|$CLOCK_OPT_OPTO_BLOCK_NAME"]} {
	redirect -tee -file ${REPORTS_DIR}/${REPORT_PREFIX}.report_congestion {route_global -congestion_map_only true}   
        redirect -tee -append ${REPORTS_DIR}/${REPORT_PREFIX}.report_congestion {report_congestion -layers [get_layers -filter "layer_type==interconnect"] -nosplit}
